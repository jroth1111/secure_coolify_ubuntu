...
6. Admin account + SSH hardening (SSH key enforcement, dedicated SSH admin group scoping, OpenSSH drop-in, cipher/MAC/KexAlgorithm restrictions, Coolify localhost Match block, config validation)
...
- `TUNNEL_MODE` (default `false`) — when true, skips WAN 80/443 UFW ALLOW rules and omits the DOCKER-USER `wan-web` ACCEPT rule
...
- `UPDATE_PROFILE` (default `security-only`) — unattended-upgrades profile: `security-only` (Ubuntu security only) or `balanced` (Ubuntu security + Ubuntu `-updates` + Docker CE stable)
...
## Firewall Model Rationale

Ubuntu supports nftables-oriented firewall stacks, but this project intentionally keeps UFW for host-interface policy and `DOCKER-USER` iptables rules for Docker/Coolify forwarding enforcement.

That is a compatibility decision for the current automation: the managed WAN allow/drop and bridge return rules live in `DOCKER-USER`, so Docker must stay on the iptables backend for this repo's enforcement model to work as designed. This is not a claim that Ubuntu's broader firewall direction is unchanged.

## Outputs

- Main log: `/var/log/bootstrap-hardening.log`
- Verification report: `/var/log/bootstrap-hardening-report.json`
- Actionable summary: `/var/log/bootstrap-hardening-summary.txt` (generated by `hardening-report`)
...
sudo sshd -T | egrep '^(port|permitrootlogin|passwordauthentication|permitemptypasswords|compression|ciphers|macs|kexalgorithms|hostkeyalgorithms|kbdinteractiveauthentication|pubkeyauthentication|authenticationmethods|allowusers|allowgroups) '
...
# Actionable summary (default-on local reporting baseline)
...
- SSH admins limited to the dedicated `coolify-ssh-admins` group in addition to the named admin user allowlist
- SSH 2FA remains optional in this rollout and is not enabled by default by the bootstrap scripts
...
- Unattended-upgrades always covers the Ubuntu security origin; `balanced` additionally includes Ubuntu `-updates` and Docker CE stable origin, while `security-only` excludes both
...
- `hardening-report.timer` active by default — writes a local-only operator summary (latest validation status, fail2ban state, recent UFW/audit activity) without claiming centralized observability
