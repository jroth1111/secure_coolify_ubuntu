...
| `DEP-06` | Deploy Gate C invokes `validate_hardening.sh --json` and reports result | `tests/unit/test_deploy_workflow_contract.bats`: `deploy: gate C runs validate_hardening.sh json` | Sufficient |
...
| `SET-05` | Setup Gate C invokes `validate_hardening.sh --json` and reports result | `tests/unit/test_setup_workflow_contract.bats`: `setup: gate C runs validate_hardening.sh json` | Sufficient |
