...
- **[5/5] Final verification** — Gate E (curl: dashboard `:8000` and websocket `:6001` reachable on Tailscale IP, both blocked on public IP), Gate F (standard mode: `https://DOMAIN` must respond 2xx/3xx; tunnel mode: `http://DOMAIN` and `http://ws.DOMAIN` must work via Tailscale, public origin `SERVER_IP:80/443` blocked, and Cloudflare host `A` records pinned to `TS_IP` DNS-only), final `validate_hardening.sh --json` run, prints summary box
