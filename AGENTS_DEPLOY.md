...
- **[4/5] DNS + Verify** — configures dashboard UFW rules, sets PUSHER_HOST/PORT/SCHEME to Tailscale realtime (`TS_IP:6001` over `http`) in tunnel mode, creates/replaces Cloudflare Tunnel, removes exact host DNS records (`DOMAIN`, `ws.DOMAIN`), and manages wildcard DNS (`*.APP_DOMAIN`, plus `*.ZONE` when different) (or A + wildcard A in standard mode)
- **[5/5] Final verification** — Gate E (curl: dashboard `:8000` and websocket `:6001` reachable on Tailscale IP, both blocked on public IP), Gate F (standard mode: `https://DOMAIN` must respond 2xx/3xx; tunnel mode: `https://DOMAIN` and `https://ws.DOMAIN` must NOT return 2xx/3xx), final `validate_hardening.sh --json` run, prints summary box
...
- **Tunnel mode**: Cloudflare terminates TLS. Public tunnel ingress routes only app wildcard hostnames (`*.APP_DOMAIN` and optional `*.ZONE`) to `localhost:80` (Traefik/coolify-proxy). Dashboard/realtime hostnames are blocked (`http_status:404`) and exact DNS host records for `DOMAIN` / `ws.DOMAIN` are removed. Coolify realtime uses Tailscale (`PUSHER_HOST=TS_IP`, `PUSHER_PORT=6001`, `PUSHER_SCHEME=http`). No origin cert.
...
| "Cannot connect to real-time service" warning in dashboard | Realtime not using Tailscale endpoint | Tunnel mode: verify `/data/coolify/source/.env` has `PUSHER_HOST=<tailscale-ip>`, `PUSHER_PORT=6001`, `PUSHER_SCHEME=http`; verify `/etc/cloudflared/config.yml` has no `localhost:6001`/`localhost:6002`/`localhost:8000` ingress routes and includes `http_status:404` deny rules for dashboard/ws hosts. Then run: `docker compose -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml up -d --force-recreate coolify soketi` |
