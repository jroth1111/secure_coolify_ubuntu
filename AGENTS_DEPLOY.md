...
- **[4/5] DNS + Verify** — configures dashboard UFW rules, sets PUSHER_HOST/PORT/SCHEME to Tailscale realtime (`TS_IP:6001` over `http`) in tunnel mode, creates/replaces Cloudflare Tunnel, configures private dashboard routes on `DOMAIN`/`ws.DOMAIN`, sets exact host DNS `A` records (`DOMAIN`, `ws.DOMAIN`) to the server `TS_IP` (DNS-only), and manages wildcard DNS (`*.APP_DOMAIN`, plus `*.ZONE` when different) (or A + wildcard A in standard mode)
- **[5/5] Final verification** — Gate E (dashboard `:8000` and websocket `:6001` reachable on Tailscale IP), Gate F (standard mode: `https://DOMAIN` must respond 2xx/3xx; tunnel mode: `http://DOMAIN` and `http://ws.DOMAIN` must work via Tailscale, and Cloudflare host `A` records pinned to `TS_IP` DNS-only), plus public-path blocking confirmation (`SERVER_IP:8000/6001` and in tunnel mode `SERVER_IP:80/443`) via automated curls in `deploy.sh` or prompted laptop confirmation in `setup.sh`; final `validate_hardening.sh --json` run; summary box printed
...
- **Tunnel mode**: Cloudflare terminates TLS. Public tunnel ingress routes only app wildcard hostnames (`*.APP_DOMAIN` and optional `*.ZONE`) to `localhost:80` (Traefik/coolify-proxy). Dashboard/realtime hostnames remain blocked in cloudflared (`http_status:404`), while exact DNS host `A` records for `DOMAIN` / `ws.DOMAIN` point to `TS_IP` with proxy disabled (Tailscale-only reachability). Coolify realtime uses Tailscale (`PUSHER_HOST=TS_IP`, `PUSHER_PORT=6001`, `PUSHER_SCHEME=http`). No origin cert.
...
| "Cannot connect to real-time service" warning in dashboard | Realtime not using Tailscale endpoint | Tunnel mode: verify `/data/coolify/source/.env` has `PUSHER_HOST=<tailscale-ip>`, `PUSHER_PORT=6001`, `PUSHER_SCHEME=http`; verify `/etc/cloudflared/config.yml` has no `localhost:6001`/`localhost:6002`/`localhost:8000` ingress routes and includes `http_status:404` deny rules for dashboard/ws hosts; verify Cloudflare `A` records for `DOMAIN` and `ws.DOMAIN` point to `TS_IP` with proxy disabled. Then run: `docker compose -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml up -d --force-recreate coolify soketi` |
