...
- **[4/5] DNS + Verify** — configures dashboard UFW rules, sets PUSHER_HOST/PORT/SCHEME for private realtime (`ws.DOMAIN:443` over `https`) in tunnel mode, creates/replaces Cloudflare Tunnel, configures private dashboard routes on `DOMAIN`/`ws.DOMAIN` for both HTTP and HTTPS, sets exact host DNS `A` records (`DOMAIN`, `ws.DOMAIN`) to the server `TS_IP` (DNS-only), and manages wildcard DNS (`*.APP_DOMAIN`, plus `*.ZONE` when different) (or A + wildcard A in standard mode)
- **[5/5] Final verification** — Gate E (dashboard `:8000` and websocket `:6001` reachable on Tailscale IP), Gate F (standard mode: `https://DOMAIN` must respond 2xx/3xx; tunnel mode: `http://DOMAIN`, `https://DOMAIN`, `http://ws.DOMAIN`, and `https://ws.DOMAIN` must work via Tailscale, and Cloudflare host `A` records pinned to `TS_IP` DNS-only), plus public-path blocking confirmation (`SERVER_IP:8000/6001` and in tunnel mode `SERVER_IP:80/443`) via automated curls in `deploy.sh` or prompted laptop confirmation in `setup.sh`; final `validate_hardening.sh --json` run; summary box printed
...
- **Tunnel mode**: Cloudflare terminates TLS. Public tunnel ingress routes only app wildcard hostnames (`*.APP_DOMAIN` and optional `*.ZONE`) to `localhost:80` (Traefik/coolify-proxy). Dashboard/realtime hostnames remain blocked in cloudflared (`http_status:404`), while exact DNS host `A` records for `DOMAIN` / `ws.DOMAIN` point to `TS_IP` with proxy disabled (Tailscale-only reachability). Coolify realtime uses private websocket hostname TLS (`PUSHER_HOST=ws.DOMAIN`, `PUSHER_PORT=443`, `PUSHER_SCHEME=https`). No origin cert.
...
| "Cannot connect to real-time service" warning in dashboard | Realtime not using private websocket hostname/TLS | Tunnel mode: verify `/data/coolify/source/.env` has `PUSHER_HOST=ws.<domain>`, `PUSHER_PORT=443`, `PUSHER_SCHEME=https`; verify `/etc/cloudflared/config.yml` has no `localhost:6001`/`localhost:6002`/`localhost:8000` ingress routes and includes `http_status:404` deny rules for dashboard/ws hosts; verify `/data/coolify/proxy/dynamic/coolify-private-dashboard.yaml` contains both HTTP and HTTPS routers for `DOMAIN` and `ws.DOMAIN`; verify Cloudflare `A` records for `DOMAIN` and `ws.DOMAIN` point to `TS_IP` with proxy disabled. Then run: `docker compose -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml up -d --force-recreate coolify soketi` |
| `TOO_MANY_REDIRECTS` on app | Resource domain using `https://` in Coolify | Change to `http://` — Cloudflare handles TLS at edge |
| App domains show Coolify dashboard instead of the app | Wildcard ingress routes to `localhost:8000` (dashboard) instead of `localhost:80` (Traefik) | Fix `/etc/cloudflared/config.yml`: wildcard `*.APP_DOMAIN` service must be `http://localhost:80`. Keep dashboard/ws host entries on `http_status:404` in cloudflared and use managed private proxy routes (`/data/coolify/proxy/dynamic/coolify-private-dashboard.yaml`) for `DOMAIN` / `ws.DOMAIN`. Then `sudo systemctl restart cloudflared`. Ensure coolify-proxy is started (Coolify UI > Servers > localhost > Proxy > Start). |
