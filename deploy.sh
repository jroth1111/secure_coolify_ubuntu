...
cf_get_zone_id() {
  ...
  # If domain has only two parts (e.g. example.com), use it directly
  local dot_count
...
phase3_docker_coolify() {
  ...
  ssh_admin_sudo 'curl -fsSL https://get.docker.com | sh' \
  ...
  ssh_admin_sudo 'curl -fsSL https://cdn.coollabs.io/coolify/install.sh | bash' \
...
phase4_binding_dns() {
  ...
  if [[ "${DEPLOY_MODE}" == "standard" ]]; then
    ...
    ssh_admin_sudo 'apt-get update -qq && apt-get install -y -qq cloudflared' 2>/dev/null \
      || {
        # Fallback: add Cloudflare repo and install
        log "Trying Cloudflare repository..."
        ssh_admin_sudo 'curl -fsSL https://pkg.cloudflare.com/cloudflare-main.gpg | tee /usr/share/keyrings/cloudflare-main.gpg >/dev/null' \
          || die "Failed to add Cloudflare GPG key"
        ssh_admin_sudo 'echo "deb [signed-by=/usr/share/keyrings/cloudflare-main.gpg] https://pkg.cloudflare.com/cloudflared $(lsb_release -cs) main" | tee /etc/apt/sources.list.d/cloudflared.list' \
          || die "Failed to add Cloudflare repository"
        ssh_admin_sudo 'apt-get update -qq && apt-get install -y -qq cloudflared' \
...
phase5_verify() {
  ...
  for (( attempt=1; attempt<=attempts; attempt++ )); do
    ts_code="$(curl -s -o /dev/null -w '%{http_code}' --connect-timeout 10 "http://${TS_IP}:8000" 2>/dev/null || echo "000")"
    pub_code="$(curl -s -o /dev/null -w '%{http_code}' --connect-timeout 5 "http://${SERVER_IP}:8000" 2>/dev/null || echo "000")"
